...
def _extractPayload(response, slaveaddress, mode, functioncode):
    ...
    #print " ".join(hex(ord(n)) for n in receivedChecksum)
    #print " ".join(hex(ord(n)) for n in response[0 : len(response) - numberOfChecksumBytes])
    #print " ".join(hex(ord(n)) for n in calculatedChecksum)
    ...
    if mode == MODE_CUSTOM:
		...
		print " ".join(hex(ord(n)) for n in response[0*8+2:8*0+8+2])
		print "out current:"
		...
		print " ".join(hex(ord(n)) for n in response[11*8+2:8*11+8+2])
		print "power on time:"
		print "".join(chr(ord(n)) for n in response[12*8+2:8*12+8+2])
		print " ".join(hex(ord(n)) for n in response[12*8+2:8*12+8+2])
		"""
	    else:
	        _checkNack(_hexdecode(response[3:5]))
